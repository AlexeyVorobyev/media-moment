...
def main():
    ...
    while True:
        ...
        mask = cv2.morphologyEx(mask, cv2.MORPH_OPEN, np.ones((10, 10)))  # erosion + dilation (remove small objects)
        mask = cv2.morphologyEx(mask, cv2.MORPH_CLOSE, np.ones((10, 10)))  # dilation + erosion (remove small holes)
